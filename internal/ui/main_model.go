...
func (m *mainModel) handleVarResolveKey(msg tea.KeyMsg) tea.Cmd {
	...
	default:
		...
		// Check for substitute-search key
		if msg.String() == config.GetKeySubstitute() {
			if m.enterSubstituteSearch() {
				// Force a clean repaint and start the textinput blink so
				// the overlay renders correctly on the very first frame.
...
// otherwise the previous input is restored. Returns a Cmd that the caller
// should propagate to bubbletea to clear any overlay artifacts.
...
func (m mainModel) renderSubstituteSearch() string {
	...
	b.WriteString(list)
	b.WriteString(strings.Repeat("\n", padding))
	b.WriteString(m.renderSubstituteInput(width))
